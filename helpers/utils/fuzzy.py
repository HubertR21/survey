    ...
    def _m_step(self, X):
        ...
        for center_id in range(self.k):
            ...
            if not np.any(center_mask):
                # The centroid of empty clusters is set to the center of everything
                if X_center is None:
...
def fuzzy_kmeans_fit_predict(X, n_clusters):
    fuzzy_kmeans = FuzzyKMeans(k=n_clusters, m=2)
    fuzzy_kmeans.fit(X)
    return fuzzy_kmeans.fuzzy_labels_
